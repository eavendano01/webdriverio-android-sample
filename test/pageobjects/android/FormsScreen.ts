export class FormsScreen {
  ...
  async typeResult() {
    await browser.waitUntil(
      async () => (await $('~input-text-result').getText()) === 'Hello World!',
      {
        timeout: 5000,
  ...
  async switchButton() {
    ...
    if (switchText !== 'Click to turn the switch OFF') {
      throw new Error(`Texto inesperado en el switch: ${switchText}`);
    }
    await browser.pause(3000);
  }
  ...
  async selectDropdown() {
    ...
    const dropdownText = await this.dropdownField.getText();
    if (dropdownText !== 'This app is awesome') {
      throw new Error(`Texto inesperado en el dropdown: ${dropdownText}`);
    }
    await browser.pause(3000);
  }
